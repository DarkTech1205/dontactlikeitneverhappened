...
// (createFontChars, updateLabel, setString, setCString). These call
// into each other internally (e.g. setString -> updateLabel ->
...
class $modify(ModOwnerButtons, LevelInfoLayer) {
    void updateSideButtons() {
        ...
        // actually a moderator - purely cosmetic navigation. We do
        // NOT touch any account/permission flag here, so this only
        // unhides a client-side button - it grants no real authority.
        if (m_starRateBtn) {
    ...
    // Intentionally NOT overriding onRateStarsMod / onRateDemon /
    // confirmOwnerDelete anymore - clicking these now calls GD's real,
    // unmodified functions. That means the actual game popups open
    // (including any real elder-mod-only options the game itself
    // shows), and whatever request they send goes to the real server
    // exactly as it would for anyone. We aren't spoofing any account
    // permission flag, so if the account genuinely isn't a real
    // mod/owner, the server's own authorization check is what decides
    // what happens next - same as it always would. If you get a
    // specific error or crash clicking one of these, send it over and
    // we'll pin down the actual popup class from that.
};
